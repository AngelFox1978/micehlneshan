...
wp_localize_script(
	...
	array(
		...
		'builderStatusData' => array(
			'status'                          => cityline_theme()->getPluginsManager()->getPluginState( cityline_get_builder_plugin_slug() ),
			'install_url'                     => cityline_theme()->getPluginsManager()->getInstallLink( cityline_get_builder_plugin_slug() ),
			'activate_url'                    => cityline_theme()->getPluginsManager()->getActivationLink( cityline_get_builder_plugin_slug() ),
			'slug'                            => cityline_get_builder_plugin_slug(),
...
<div class="cityline-admin-big-notice--container">
	<div class="logo-holder">
		<h2><?php echo sprintf( __( 'Choose one of the %s predefined designs to start with:', 'cityline' ), ucfirst( get_stylesheet() ) ); ?></h2>
	</div>
	<div class="content-holder">
		<ul class="predefined-front-pages">
            ...
            $front_page_designs =  array(
                array(
                    'name'    => __( 'Modern', 'cityline' ),
                    'index'   => 1,
                    'preview' => 'front-page-1.jpg',
                ),

                array(
                    'name'    => __( 'Fullscreen', 'cityline' ),
                    'index'   => 2,
                    'preview' => 'front-page-2.jpg',
                ),

                array(
                    'name'    => __( 'Classic', 'cityline' ),
                    'index'   => 3,
            ...
            foreach ($front_page_designs as $cityline_design_index => $design ) : ?>
				<?php
					$cityline_design_selected = $cityline_design_index === 0 ? 'selected' : '';
				...
				<li data-index="<?php echo esc_attr( $design['index'] ); ?>"
					class="<?php echo $cityline_design_selected; ?>">
					...
					<div class="predefined-front-page-card">
						...
						<div class="front-page-design-wrapper">
							<div class="design-preview-image"
								 style="background-image: url(<?php echo esc_attr( cityline_theme()->getAssetsManager()->getBaseURL() . "/images/{$preview_image_name}" ); ?>)"
							></div>
						</div>
