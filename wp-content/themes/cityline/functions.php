...
function cityline_register_components( $components ) {
...
Hooks::prefixed_add_filter( 'components', 'cityline_register_components', 20 );
Theme::load(
	array(
 ...
 * @return Theme
 */
function cityline_theme() {
	return Theme::getInstance();
}

function cityline_assets() {
	return cityline_theme()->getAssetsManager();
}
...
add_action(
	'after_setup_theme',
	function() {
		cityline_theme()->register_menus(
			array(
				'header-menu' => esc_html__( 'Header Menu', 'cityline' ),
				'footer-menu' => esc_html__( 'Footer Menu', 'cityline' ),
			)
		);
	},
	1
...
if ( ! apply_filters( 'kubio_is_enabled', false ) ) {
	cityline_assets()
		...
		->registerStylesheet( 'cityline-theme', '/theme/theme.css' );
	cityline_assets()->loadLocalGoogleFonts();
	//	foreach ( Defaults::get( 'fonts', array() ) as $family => $variants ) {
	//		cityline_assets()->addGoogleFont( $family, $variants );
	//	}

	add_action( 'wp_enqueue_scripts', 'cityline_print_color_scheme', 0 );
} else {
	cityline_assets()
		->registerStyle( 'cityline-theme', cityline_assets()->getBaseURL() . '/theme/fse-base-style.css' );
	Hooks::prefixed_add_filter( 'skip_google_fonts', '__return_true' );
}

function cityline_theme_add_woocommerce_support() {
	add_theme_support(
		'woocommerce',
		array(
			'product_grid' => array(
...
add_action( 'after_setup_theme', 'cityline_theme_add_woocommerce_support' );

add_filter( 'kubio/activation/after_activation_redirect_url', 'cityline_after_activation_redirect_url' );
function cityline_after_activation_redirect_url( $url ) {
...
function cityline_get_builder_plugin_slug() {
...
function cityline_theme_plugins( $plugins ) {

	$plugin_slug = cityline_get_builder_plugin_slug();

	$plugins[ $plugin_slug ] = array(
		'name'        => $plugin_slug === 'kubio-pro' ? __( 'Kubio PRO', 'cityline' ) : __( 'Kubio', 'cityline' ),
...
Hooks::prefixed_add_filter( 'theme_plugins', 'cityline_theme_plugins' );
...
Hooks::prefixed_add_action(
	...
	function ( $slug ) {
		...
		if ( $slug === cityline_get_builder_plugin_slug() ) {
				...
				<div class="kubio-admin-row get-started-imported notice notice-success">
					<div>
						<p class="imported-title">
							...
							echo esc_html(
								sprintf(
									__( '%s design has been successfully imported!', 'cityline' ),
									cityline_theme()->getName()
								)
							);
							?>
						</p>
						<p class="imported-subtitle">
							...
							echo esc_html(
								sprintf(
									__( '%s design has been successfully imported! You can take a look at your new design or start editing it', 'cityline' ),
									cityline_theme()->getName()
								)
							);
							?>
					...
					<div class="button imported-view-site-button">
						<a href="<?php echo esc_url( site_url() ); ?>">
							<?php echo esc_html( __( 'View site', 'cityline' ) ); ?>
						</a>
...
function cityline_try_online_url() {
	return 'https://kubiobuilder.com/go/try-theme/' . get_template();
}

add_filter( 'kubio/editor-try-online/url', 'cityline_try_online_url', 10 );

function cityline_render_header_style() {

	?>
	<base target="_top">
	<?php

	cityline_theme()->get( 'css' )->render();
}
add_action( 'wp_head', 'cityline_render_header_style', 100 );
...
Hooks::prefixed_add_filter(
	...
	function( $translations ) {
		$translations['customize_preview_overlay_message']  = __( 'These features are part of the Kubio Page Builder plugin. Using them will install the plugin.', 'cityline' );
		$translations['customize_preview_overlay_button_1'] = __( 'Edit this section', 'cityline' );
		$translations['customize_preview_overlay_button_2'] = __( 'Replace this section', 'cityline' );
	...
	function() {
		return array(
			StarterContent::HOME_SLUG => array(
				'post_title' => __( 'Home', 'cityline' ),
				'in_menu'    => true,
			),

			// StarterContent::ABOUT_SLUG => array(
			// 	'post_title' => __( 'About', 'cityline' ),
			// 	'in_menu'    => true,
			// ),

			StarterContent::BLOG_SLUG => array(
				'post_title' => __( 'Blog', 'cityline' ),
				'in_menu'    => true,
			),

			// StarterContent::CONTACT    => array(
			// 	'post_title' => __( 'Contact', 'cityline' ),
			// 	'in_menu'    => true,
			// ),
